...
struct
  ...
  fun x -- y = BNS.sub (x, y)
  fun x ** y =
    if (length x = 1 andalso nth x 0 = ZERO) orelse 
       (length y = 1 andalso nth y 0 = ZERO)
      then singleton ZERO
    else if (length x = 1 andalso nth x 0 = ONE) andalso 
            (length y = 1 andalso nth y 0 = ONE)
            then singleton ONE
    else
      let
        ...
        (*q & s indicates the lower, while p & r the higher*)
        ...
        val (s,r) = case showt y_full of
          ...
          | ELT x0       => (singleton x0  , singleton ZERO)
          | NODE (s',r') => (s',r')
        val move = length s     (*attention on the function 'showt'!, the length maybe odd*)
