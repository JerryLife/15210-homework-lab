    ...
    let
      ...
      fun fs (V : vertex seq, E : (vertex * (vertex * int)) seq, C : int seq) (seed : Rand.rand) =
        let
          ...
          val v_zip = collect Int.compare (map rev_v edges_chosen)
            (*get the sum of weight of neighbors for each destination vertex*)
          val sum_ws = map (fn (u,zips) => (u,reduce op+ 0 (map (fn x => #2 x) zips))) v_zip
      ...
      val rst = fs (vertices, edges, credits) (Rand.fromInt 4843)
    in
      get_map rst
    end
